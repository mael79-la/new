...
function guardarEnNavegador() {
    try {
        ...
        alert("¡Error! La imagen es demasiado pesada para el navegador. Intenta con un logo más chico o de menor resolución.");
    }
}
...
// CORREGIDO: Muestra correctamente el nombre del archivo seleccionado
fileInput.addEventListener("change", (e) => {
    const fileNameElement = document.getElementById("file-name");
    if(e.target.files.length > 0) {
        fileNameElement.innerText = e.target.files[0].name; // <--- Corregido propiedad [0].name
    } else {
        fileNameElement.innerText = "Ningún archivo seleccionado";
    }
});

// Acción Directa del Botón de Descarga
btnDescargar.addEventListener("click", () => {
    if (!appSeleccionadaActual) return;
...
uploadForm.addEventListener("submit", (e) => {
    ...
    const archivosImagen = fileInput.files;
